...
export default function QuoteForm({ compact = false }: { compact?: boolean }) {
  ...
  function handleSubmit(e: FormEvent) {
    ...
    const message = `Hi NMC Technology, my name is ${name}. I'd like a free solar quote for a ${property.toLowerCase()} (${propertyType.toLowerCase()} property). My phone number is ${phone}${
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <label className="flex flex-col gap-1.5 text-sm font-medium text-paper/70">
          ...
          <input
            ...
            className="rounded-xl border border-paper/15 bg-paper/[0.06] px-4 py-3 text-[15px] text-paper placeholder:text-paper/35 outline-none transition-colors focus:border-gold"
          />
          <datalist id="quoteform-town-options">
            {TOWNS.map((option) => (
              <option key={option} value={option} />
            ))}
          </datalist>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="flex flex-col gap-1.5 text-sm font-medium text-paper/70">
          ...
          <input
            ...
            className="rounded-xl border border-paper/15 bg-paper/[0.06] px-4 py-3 text-[15px] text-paper placeholder:text-paper/35 outline-none transition-colors focus:border-gold"
          />
          <datalist id="quoteform-city-options">
            {CITIES.map((option) => (
              <option key={option} value={option} />
            ))}
          </datalist>
        ...
        <label className="flex flex-col gap-1.5 text-sm font-medium text-paper/70">
          Country
          <input
            list="quoteform-country-options"
            required
            value={country}
            ...
            className="rounded-xl border border-paper/15 bg-paper/[0.06] px-4 py-3 text-[15px] text-paper placeholder:text-paper/35 outline-none transition-colors focus:border-gold"
          />
          <datalist id="quoteform-country-options">
